...
def jma_numba(
    ...
    """Numba accelerated Jurik Moving Average (JMA) calculation."""
    m = close.shape[0]
...
def jma(close, length=None, phase=None, offset=None, **kwargs):
    ...
    # Calculate JMA using Numba
    jma_values = jma_numba(
        close.values,
        length,
        phase,
        offset,
    ...
    jma_values[0 : _length - 1] = np.nan
    jma = Series(jma_values, index=close.index)
